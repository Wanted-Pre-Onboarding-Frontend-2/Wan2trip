...
- 무한스크롤 api를 사용해 호텔 데이터를 10개씩만 호출합니다. 리스트가 출력된 페이지에서 사용자가 스크롤을 내려 마지막 카드에 도달하면 비동기로 옵저버를 실행해줍니다.

## Virtual scroll

- Virtual scroll를 이용해 viewport의 카드들만 dom tree에 렌더하게끔 구현하였다.
  - dom tree에 붙여진 card list의 개수를 줄임으로써 렌더링 시간을 줄인다(특히 모바일에서 야기될 수 있는 퍼포먼스 이슈를 제거하게 된다)
  - 무한스크롤 limit인 10을 node padding으로 하여, 사용자가 끊김없이 컨텐츠를 볼 수 있도록 구현
  - 렌더링하고자 하는 item list와 virtual scroll간의 종속성을 제거함으로써, 다른 컴포넌트도 해당 virtual scroll을 사용할 수 있도록 구현
